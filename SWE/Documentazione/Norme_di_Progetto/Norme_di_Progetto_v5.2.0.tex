...
\midrule
4.0.3 & 2014-04-10 & Michele Maso (AM) & Stesura attivit\`{a}.\\
\midrule
...
\subsubsection{Responsabile di Progetto} %2.1
...
Il \gloss{Programmatore} è responsabile delle attivit\`{a} di Codifica e delle componenti di ausilio
...
\item \grassetto{Piano di Qualifica}: \emph{Piano\_di\_Qualifica\_v5.2.0};
\item \grassetto{Piano di Progetto}: \emph{Piano\_di\_Progetto\_v5.2.0}.
\end{itemize}
...
All'inizio di ciascuna delle fasi delineate nel \emph{Piano di Progetto v5.2.0}, il Responsabile di Progetto deve realizzare un \gloss{diagramma di \gloss{Gantt}} su \gloss{Redmine}, come descritto in sezione \ref{1} di questo documento, per tracciare e gestire le attivit\`{a} dei processi utilizzati dal progetto.\\
...
\begin{itemize}
\item \grassetto{Nuovo:} Il compito è stato creato ed è in attesa di essere assegnato;
\item \grassetto{In Elaborazione:}  Il compito è stato assegnato ed è in lavorazione;
...
\paragraph{Definizione delle classi} \hfill \\
...
\paragraph{Definizione dei moduli} \hfill \\
\label{}
...
\begin{itemize}
\item \grassetto{Nome: }specifica il nome del modulo, deve essere preceduto da "@" e deve essere il nome del paragrafo corrispondente al modulo;
\item \grassetto{Descrizione: }descrive in maniera testuale la funzionalità del modulo;
\item \grassetto{Metodi: } Devono essere descritti i metodi interni al modulo. Dopo il metodo devono essere elencati i parametri in ingresso tra parentesi tonde.
...
All'interno della definizione di ogni modulo, il riferimento ad altri moduli dovrà essere identificato da una notazione in \grassetto{grassetto}. Quando vengono menzionati i metodi, invece, viene riportato il loro nome in \textit{corsivo}.
...
\item \grassetto{Stato: }lo stato in questo caso sarà uguale per tutti, ovvero D.S. (da sviluppare), in quanto i test verranno eseguiti più avanti.
\end{itemize}
...
\begin{itemize}
\item \grassetto{Test: }codice univoco che identifica il test. Per i test d'integrazione si inizierà con TI;
\item \grassetto{Descrizione: } descrizione testuale del test;
...
\begin{itemize}
\item \grassetto{Test: }codice univoco che identifica il test. Per i test di unità si inizierà con TU;
\item \grassetto{Descrizione: } descrizione testuale del test;
\item \grassetto{Metodi testati: } elenca le funzionalità testate;
\item \grassetto{Stato: }lo stato dovrà essere D.S. nel caso in cui il test non sia ancora stato sviluppato.Nel caso in cui invece il test sia stato eseguito lo stato potrà essere superato oppure fallito.
\end{itemize}
...
\paragraph{Comunicazioni interne formali}
Le comunicazioni interne formali appartengono all'area di organizzazione generale del gruppo. 
\subparagraph{Procedure}
Ogni \gloss{email} inviata a tale indirizzo, per segnalazioni di imprevisti o convocazioni di riunioni, verrà inoltrata alla email personale di ogni componente nel gruppo.
...
\subparagraph{Gruppo \gloss{Facebook}} \hfill \\
...
\paragraph{Procedure}
L' email può essere usata soltanto dal Responsabile di Progetto e verrà utilizzata per tutte le comunicazioni che il gruppo terrà con l'\gloss{ambiente} esterno.
\paragraph{Strumenti}
...
\subparagraph{Verificare la copertura minima} \hfill \\
...
\paragraph{Tracciamento componente-test d'integrazione} \hfill \\
...
\paragraph{Norme tipografiche e stili di testo} \hfill \\
\label{5.4}
...
\paragraph{Sigle e abbreviazioni} \hfill \\
\label{5.5}
...
\subparagraph{Avanzamento di versione} \hfill \\
...
Per copiare in locale il contenuto del repository bisogna eseguire il comando \textit{git pull} nella cartella che contiene il repository locale.
