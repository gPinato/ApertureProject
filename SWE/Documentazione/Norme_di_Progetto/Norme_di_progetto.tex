...
Redige il \emph{Piano di Progetto} e collabora alla stesura del \emph{Piano di Qualifica}, per quanto riguarda la sezione di pianificazione delle attività.
...
\item La stesura di una specifica recisa, completa e comprensibile dal Proponente, dal Committente e dal Progettista.
\end{itemize}
...
\label{}
Per semplificare e automatizzare il tracciamento dei requisiti è stato realizzato un database relazionale che associa ad ogni requisito la sua sorgente e gli eventuali moduli che lo implementano. E' inoltre possibile effettuare la ricerca inversa e, partendo da un componente od un use case, vedere i requisiti associati.
...
\item in ogni caso, non iniziare una definizione con "è un".
\end{itemize}
...
dalla versione X.0.Z alla versione X.1.0. E' compito del responsabile assegnare ai Verificatori il ticket di verifica per il documento in questione.
...
\item Segnalazione errori: Alla fine del processo di verifica, i Verificatori devono creare un ticket per ogni errore da riparare, come specificato nella sezione 8.4.3.
\end{itemize}
...
E' inoltre loro compito assicurarsi che i moduli del sistema abbiano dimensione e contenuto adatti ad essere realizzati da un singolo Programmatore.
\subsection{Codifica}
\label{}
