...
\begin{abstract}
\begin{center}
Questo documento si propone di effettuare un'analisi di fattibilità dei capitolati proposti.
\end{center}
\end{abstract}
...
\label{2}
Il capitolato proposto da \Prop{} propone di sviluppare un \gloss{framework} ad alto livello che permetta la facile creazione di pagine \gloss{web} per la visualizzazione e la modifica di dati provenienti da \gloss{MongoDB}.
\subsection{Studio del dominio applicativo} %2.1
\label{2.1}
Sempre più frequentemente agli sviluppatori viene richiesto di fornire in modo rapido strumenti per la gestione di dati \gloss{business} accessibili anche da utenti inesperti.
...
\label{2.2}
Il dominio applicativo principale saranno le applicazioni web, principalmente \gloss{server} side, e la gestione di \gloss{database}.
Nello specifico le principali tecnologie che verranno utilizzate saranno le seguenti:
\begin{itemize}
\item \gloss{Node.js} per la realizzazione della struttura server side;
\item \gloss{Express} per la realizzazione dell'\gloss{infrastruttura} della web application generata;
...
\label{2.3}
Come si evince dal capitolato, gli strumenti per la gestione dinamica e veloce di pagine web per l'amministrazione di database \gloss{NoSQL} sono molteplici e in rapida espansione a causa della grande richiesta del mercato, che cerca sempre di più di avvicinare lo \gloss{sviluppatore} e l'\gloss{utente} finale in modo da facilitare al primo il lavoro, al secondo l'utilizzo. Le potenzialità si intravedono quando nel capitolato viene richiesto che lo stack tecnologico si basi su Node.js, applicazione per lo sviluppo web molto potente ed efficiente, e su MongoDB, database non \gloss{relazionale} che permette una maggiore semplicità del processo di modellazione dei dati, una migliore propensione allo \gloss{scaling orizzontale} del database e un controllo maggiore della disponibilità dei dati rispetto a modelli relazionali.
\subsection{Studio delle criticità}
\label{2.4}
Le principali criticità che si intravedono fin da subito sono la complessità che può avere l'estrarre dati da un database NoSQL e presentarli in forma semplice all'utente, nascondendo tutta la complessità gestionale dietro al framework.
...
Un ultima grande criticità la pone la creazione del DLS, che dovrà essere implementato con estrema precisione, eliminando qualsiasi ambiguità e con un  livello di astrazione adeguato allo scopo.
...
\item Dipendenza da \gloss{software} precedentemente esistenze.
\end{itemize}
\subsubsection{Potenziali criticità}
Era richiesto lo sviluppo di un plug-in per un \gloss{programma} già esistente, pertanto il fatto di doversi interfacciare con qualcosa di completamente sconosciuto avrebbe potuto essere fonte di grandi problemi.
...
\item Nessun componente del team era particolarmente interessato all'\gloss{ambiente} mobile.
\end{itemize}
...
\item La larga \gloss{scala} su cui doveva essere implementato il progetto avrebbe reso molto difficili i test di funzionamento.
\end{itemize}
\subsubsection{Potenziali criticità}
