...
\end{lstlisting}
JK Non so dimmi tu
L'utente può definire delle query per restringere ulteriormente la selezione dei dati specificati in index-columns

Per quanto riguarda il campo populate, è un array di oggetti che specifica il nome di collections esterne su cui si vogliono prelevare ulteriori dati.
Ad esempio se stiamo descrivendo la collection Teams, dentro columns possiamo definire colonne presenti nei documenti della collection teams, come nome\_squadra, numero\_giocatori.
Se però vogliamo aggiungere una colonna con il nome del coach dobbiamo prima usare populate sulla collection "coaches" per poter pescare i dati..
Quindi si definisce populate [{collection: 'coaches', key: 'coach' }]
e poi nelle colonne si può usare "coach.name" che va a prendere il nome del coach nella collection "coaches".
