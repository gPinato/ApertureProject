...
\begin{itemize}
\item \grassetto{Correttore \LaTeX :}lo strumento utilizzato, descritto nella sezione delle Norme di Progetto, per la scrittura dei documenti permette di identificare le parole che vengono scritte in maniera sbagliata; in questo modo ci si può accorgere dell'errore e procedere alla correzione mentre si sta scrivendo;
\item \grassetto{Scrittura frasi: }si cerca di scrivere frasi corte e semplici e con termini di uso comune per ottenere un valore accettabile dell'indice di Gulpease, descritto nella sezione 2.10.2 di questo documento;
\item \grassetto{Scrittura paragrafo: }ogni volta che verrà scritto un paragrafo, si procederà subito alla sua verifica.
\end{itemize}
Nell'attività di codifica, appartenente al processo di sviluppo, verranno adottate le seguenti procedure proattive:
\begin{itemize}
\item \grassetto{Ambiente di sviluppo: }l'ambiente di sviluppo, che verrà utilizzato nel prossimo periodo di tempo dedicato alla codifica, permetterà di segnalare, mentre si sta scrivendo, degli errori ortografici; questo permette di correggerli mentre si sta scrivendo del codice;
\end{itemize}
