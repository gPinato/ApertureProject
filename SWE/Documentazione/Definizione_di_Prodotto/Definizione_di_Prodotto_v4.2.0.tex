...
\item[Inizializzazione] \hfill \\
 \begin{description}
  \item[init():] \hfill \\ 
  accetta app (l'applicazione di \textit{Express}) come parametro, si occupa dell'inizializzazione del \textit{dispatcher}. 
 \end{description}
\end{description}
 ...
 \item[Metodi] \hfill \\
  \begin{description}
    \item[initFrontController()]\hfill \\
  	\'{E} la funzione di inizializzazione del frontController ed indica di reindirizzare le richieste del Client al \textit{@dispatcher} per gestirle. Accetta  app (l'applicazione di Express) come parametro.
  \end{description}
\end{description}
