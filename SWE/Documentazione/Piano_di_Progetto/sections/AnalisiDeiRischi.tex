...
\item \grassetto{Livello di rischio}: alto;
\item \grassetto{Contromisure}: i componenti del gruppo si impegnano per istruirsi sull'utilizzo delle tecnologie richieste, partecipando a seminari specifici e utilizzando materiale e documentazione reperibile sul \gloss{Web} o fornite l'Amministratore. Questi apprendimenti saranno effettuati in tempi brevi, per non compromettere e ritardare il proseguimento del progetto. i componenti del gruppo hanno dedicato delle ore pianificate il fine settimana per studiare queste tecnologie necessarie;
...
\subsection{Cambio nelle tecnologie}
\begin{itemize}
...
\item \grassetto{Contromisure}: il gruppo dovrà tenersi costantemente informato sugli aggiornamenti delle versioni dei software. In caso di nuove release, verranno effettuati dei test per garantire la compatibilità del codice prodotto;
...
\item \grassetto{Contromisure}: se il rischio si verifica, sarà compito del gruppo adeguarsi ai nuovi requisiti imposti dal Proponente. Nel caso specifico, saranno stanziate ore ulteriore di Analisi e quindi l'assegnamento di uno o più Analisti per adattare i requisiti;
