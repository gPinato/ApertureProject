...
\begin{description}
 ...
 \begin{description}
  ...
  \item[delete\textunderscore document()] \hfill
    \begin{description}
  	\item[Descrizione] \hfill \\ 
 Funzione che elimina il profilo identificato dall'indice \textit{index}. Effettua una richiesta REST di rimozione alla risorsa gestita da UserEditService fornendo come argomenti l'identificativo della Collection correntemente visualizzata e l'indice del Document da eliminare. \\
  Se l'eliminazione ha successo viene lanciato l'aggiornamento della vista corrente con il Document eliminato, altrimenti si richiama la callback di errore.
      \end{description}
...
\begin{description}
 ...
 \item[Utilizzo] \hfill \\
 Viene utilizzato come astrazione della risorsa Collection, su cui il \grassetto{@CollectionController} invocherà delle richieste RESTful aggiungendo i parametri necessari. La risorsa fornita conterr\'{a} i Document ad essa correlati e quest'ultimi verranno visualizzati in base alle opzioni della richiesta RESTful.
...
\begin{description}
 ...
 \end{itemize}
  ...
  \textit{id} uguale a quello utilizzato per la ricerca. Inoltre, oltre alla visualizzazione espressa tramite il metodo \textit{query} realizzato con una richiesta \textit{GET} RESTful, descrive che le richieste di \emph{update} vengano effettuate
 con richieste di \textit{PUT}, mentre le richieste di \textit{remove} useranno il metodo http \textit{DELETE}.
\end{description}
