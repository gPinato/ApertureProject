...
Si è deciso di fissare come obiettivo il raggiungimento del terzo livello dello standard sopracitato, ovvero established. Per cercare di ottenere questo i processi vengono definiti e organizzati, basandosi su standard di riferimento, utilizzando risorse appropriate per il raggiungimento degli obiettivi.
...
Per cercare di realizzare e progettare un prodotto software, in accordo con specifiche e standard definiti, ed essere privo di non conformità o difetti, è necessario usare lo standard ISO/IEC 9126\footnote{vedi appendice, sezione A.3}, il quale redige e descrive obiettivi qualitativi e fornisce delle linee guida d'utilizzo di metriche al fine di tracciare il progresso nel miglioramento continuo di processo e prodotto. Il gruppo si è imposto di soddisfare le seguenti caratteristiche dello standard sopracitato, secondo le metriche stabilite nella sezione Metriche software(???)
\begin{itemize}
\item \grassetto{Funzionalità}
\item \grassetto{Affidabilità}
\item \grassetto{Efficienza}
\item \grassetto{Usabilità}
\item \grassetto{Manutenibilità}
\item \grassetto{Portabilità}
\end{itemize}
\subsubsection{Obiettivi qualità documentazione}
Il gruppo \NomeGruppo{} si è imposto di produrre documentazione di qualità, data la scarsa inesperienza nel produrre documenti di questo tipo. Proprio per questo motivo è stato introdotto l'obiettivo di produrre testo caratterizzato da una buona leggibilità.


\section{Obiettivi e metriche}
In questa sezione verranno elencati gli obiettivi che il gruppo intende raggiungere. Tali obiettivi saranno poi seguiti da:
\begin{itemize}
\item \grassetto{Metrica}: ove possibile, sarà specificata una metrica per rendere l'obiettivo quantificabile; ogni metrica avrà:
\begin{itemize}
...
\end{itemize}
\item \grassetto{Strategia}: procedimento tramite il quale si raggiunge l'obiettivo prefissato. La descrizione completa della strategia sarà possibile consultarla in sezione ???.
\end{itemize}

\subsection{Processi}
Per raggiungere l'obiettivo prefissato dei processi, bisognerà:
\begin{itemize}
\item Pianificare dettagliatamente i processi;
\item Pianificare le risorse da utilizare.
\end{itemize}
...
\subsection{Software}
\label{2.9.3}
Di seguito verranno elencati gli obiettivi software.
\begin{itemize}
\item \grassetto{Funzionalità}: il prodotto software deve soddisfare tuti i requisiti funzionali, tutti i reauisiti obbligatori trovati e di conseguenza fornire le funzionalità attese;
\begin{itemize}
\item \grassetto{Obiettivo Sufficiente}:[100\% soddisfacimento requisiti obbligatori ]
\item \grassetto{Obiettivo Ottimale}:[100\% soddisfacimento di tutti i requisiti individuati]
\end{itemize}
Verrà utilizzata la strategia Prodotto Funzionale.
\item \grassetto{Affidabilità}: il prodotto software dovrà essere in grado di funzionare anche in condizioni non ottimali; ad esempio non devono essere prodotti risultati non corretti e la gestione degli errori deve essere trattata in maniera ottimale; verrà misurata la quantità di esecuzioni andate a buon fine. Verranno utilizzate le seguenti metriche:
\begin{itemize}
\item Complessità Ciclomatica;
\item Linee di codice per funzione;
\item Numero di messaggi di errore;
\item Numero di esecuzioni andate a buon fine.
\end{itemize}
Strategia adottata: Prodotto Affidabile.
\item \grassetto{Efficienza}: il prodotto software deve consumare le risorse in maniera adeguata e ridurre al minimo i tempi di attesa o i tempi di risposta per un qualsiasi risultato;
\begin{itemize}
\item \grassetto{Obiettivo Sufficiente}:[]
\item \grassetto{Obiettivo Ottimale}:[]
\end{itemize}
\item \grassetto{Usabilità}: essendo legata al soggetto utente, non è facilmente verificabile e misurabile in modo meccanico. Questa caratteristica verrà verificata solamente attraverso l'uso del prodotto;
\begin{itemize}
\item \grassetto{Obiettivo Sufficiente}:[]
\item \grassetto{Obiettivo Ottimale}:[]
\end{itemize}
\item \grassetto{Manutenibilità}: deriva da una buona progettazione architetturale, ma soprattutto da un basso accoppiamento delle componenti, vedi sezione....
\begin{itemize}
\item \grassetto{Obiettivo Sufficiente}:[]
\item \grassetto{Obiettivo Ottimale}:[]
\end{itemize}
...
\subsubsection{Metriche aggiuntive}
Di seguito verranno riportate delle metriche ritenute le più efficaci, per raggiungere obiettivi di qualità software.
...
\section{Strategie}
Verranno di seguito riportate le strategie che il gruppo adotta per il raggiungimento della qualità. Alcune strategie fanno riferimento agli obiettivi riguardanti le metriche individuate, mentre altre strategie fanno riferimento agli obiettivi generali non quantificabili.

\subsection{Strategie per obiettivi quantificabili}
\begin{itemize}
\item \grassetto{Strategia Classe accettabile}: ogni volta che si realizza una classe si controlla il numero di attributi che essa contiene, cercando di soddisfare l'obiettivo ottimale della metrica corrispondente.
\end{itemize}

\subsection{Strategie per obiettivi non quantificabili}
\begin{itemize}
\item \grassetto{Strategia Prodotto Funzionale}: per quanto riguarda la funzionalità del prodotto, verrà prodotta una tabella di tracciamento che mostra tutti i requisiti obbligatori. Per il raggiungimento di questo obiettivo il prodotto finale dovrà soddisfare tutti i requisiti mostrati in tabella.
\item \grassetto{Strategia Prodotto Usabile}: per quanto riguarda l'usabilità del prodotto, si cercherà di realizzare il prodotto pensando di farlo il più semplice possibile, ponendo maggiore attenzione all'interazione che esso avrà con chi lo usa.
\item \grassetto{Strategia Prodotto Manutenibile}: per quanto riguarda la manutenibilità del prodotto, ogni volta che verranno progettate delle componenti si procederà al calcolo dell'accoppiamento afferente ed efferente, cercando di rientrare nel valore ottimale corrispondente.
\end{itemize}
...
\input{./Piano_di_qualifica_appendice_C.tex}
