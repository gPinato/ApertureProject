...
Per ultima cosa, il \textit{dispatcher} gestisce eventuali richieste di risorse inesistenti o errate reindirizzando il chiamante alla pagina iniziale del Client AngularJS.
 ...
 \begin{itemize}
 %\item get('/api/collection/list', passport.checkAuthenticated, datamanager.sendCollectionsList);
 %\item get('/api/collection/:col_id', passport.checkAuthenticated, datamanager.sendCollection);
 %\item get('/api/collection/:col_id/:doc_id', passport.checkAuthenticated, datamanager.sendDocument);
 %\item get('/api/collection/:col_id/:doc_id/edit', passport.checkAuthenticatedAdmin, datamanager.sendDocumentEdit);
 %\item put('/api/collection/:col_id/:doc_id/edit', passport.checkAuthenticatedAdmin, datamanager.updateDocument);
 %\item delete('/api/collection/:col_id/:doc_id/edit', passport.checkAuthenticatedAdmin, datamanager.removeDocument);
 %sto bestemmiando allegramente con le signature dei metodi: sono troppo lunghe! devo inventarmi qualcosa.
 %probabilmente bisognerà fare una tabella, domani ne parlo con Andrea.
 \end{itemize}
...
\subsubsection{@frontController}		%da rivedere
\begin{description}
 \item[Descrizione] \hfill \\
 ...
 \'{E} la funzione di inizializzazione del frontController ed indica l'utilizzo del modulo \textit{dispatcher} per gestire le richieste del Client.
 \end{itemize}
\end{description}
...
\begin{description}
 ...
 \begin{description}
 \item[init()] \hfill \\
 Si occupa dell'inizializzazione dei database. Inizializza lo userDBManager, aggiunge l'amministratore di default del sistema e inizializza il dataDBManager.
 \end{description}
\end{description}
...
\item[Funzionalità esportate] \hfill \\
 \begin{description}
 \item[init()] \hfill \\
 Legge la cartella contenente le collectionData dei DSL e per ogni file inizializza il modello di mongoose come elemento dell'array modelArray che poi esposta con il nome di model.
 \end{description}
\end{description}
