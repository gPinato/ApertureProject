...
\begin{description}
	...
	\begin{description}
		...
		Funzione utilizzata per calcolare il range di indici di pagine da visualizzare.
		\item[\$scope.prevPage()] \hfill \\
		Funzione che permette di ottenere dal Server i dati del precedente gruppo di Document nella Collection.
		\item[\$scope.DisablePrevPage()] \hfill \\
		Funzione che permette la disabilitazione del pulsante di selezione della pagina precedente se si è già nella prima pagina della Collection.
		\item[\$scope.nextPage] \hfill \\
		Funzione che permette di ottenere dal Server i dati del prossimo gruppo di Document nella Collection.
		\item[\$scope.DisableNextPage] \hfill \\
		Funzione che permette la disabilitazione del pulsante di selezione della pagina successiva se si è già nell'ultima pagina della Collection.
		\item[\$scope.toPage(index)] \hfill \\
		Funzione che permette di saltare alla visualizzazione della pagina di Document identificata dal numero intero presente in \textit{index}.
	\end{description}
