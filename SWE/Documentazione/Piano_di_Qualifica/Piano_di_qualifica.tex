...
\item \grassetto{Metrica o Misura}: ove possibile, sarà specificata una metrica o una misura per rendere l'obiettivo quantificabile; successivamente in sezione ??? sarà fornita una descrizione di ogni metrica e di ogni misura, specificando anche:
\begin{itemize}
...
\end{itemize}
Sono state scelte due misure che si basano sui costi e i tempi spesi per un processo, per renderlo quantificabile. Tali misure sono:
\begin{itemize}
\item \grassetto{Schedule Variance(SV)}: vedi sezione ???;
\item \grassetto{Budget Variance(BV)}: vedi sezione ???;
\end{itemize} 
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Prodotto Funzionale(vedi sezione).
\item \grassetto{Misura o metrica utilizzata}: Soddisfacimento requisiti funzionali e obbligatori(vedi sezione ???);
\end{itemize}
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Prodotto Affidabile(vedi sezione ???).
...
\begin{itemize}
\item \grassetto{Strategia adottata}: 
\item \grassetto{Misura o metrica utilizzata}:
\end{itemize}
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Prodotto Usabile(vedi sezione???).
\item \grassetto{Misura o metrica utilizzata}:
\begin{itemize}
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Prodotto Manutenibile (vedi sezione ???).
\item \grassetto{Misura o metrica utilizzata}:
\begin{itemize}
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Prodotto Portabile(vedi sezione???).
\item \grassetto{Misura o metrica utilizzata}:
\begin{itemize}
...
\begin{itemize}
\item \grassetto{Strategia adottata}: Testo leggibile.
\item \grassetto{Misura o metrica utilizzata}: Indice di Gulpease(vedi sezione???);
\end{itemize}
\end{itemize} 
...
\section{Strategie}
Verranno di seguito riportate le strategie che il gruppo adotta per il raggiungimento della qualità. Alcune strategie fanno riferimento agli obiettivi riguardanti le metriche individuate, mentre altre strategie fanno riferimento agli obiettivi non quantificabili. Inoltre verranno fornite delle strategie di verifica.
...
Per l'affidabilità del prodotto, si cercherà di scrivere del codice con il minor numero possibile di rami logici. Se questo non risulta possibile, bisogna dividere in più metodi. Prima di effettuare un committ bisogna assicurarsi che quanto scritto fino a quel momento rientri nei range stabiliti delle metriche utilizzate.

\subsection{Prodotto Efficiente}

\subsection{Prodotto Usabile}
Per quanto riguarda l'usabilità del prodotto, si cercherà di realizzarlo pensando di farlo il più semplice possibile, ponendo maggiore attenzione all'interazione che esso avrà con chi lo usa. Inoltre il manuale utente che verrà rilasciato insieme al prodotto dovrà essere il più semplice possibile e di facile comprensione; inoltre dovrà avere un buon valore di indice di leggibilità.

\subsection{Prodotto Manutenibile}
Per quanto riguarda la manutenibilità del prodotto, ogni volta che verranno progettate delle componenti si procederà al calcolo dell'accoppiamento afferente ed efferente, cercando di rientrare nel valore ottimale corrispondente. Inoltre prima di effettuare un commit, ogni pezzo di codice scritto deve soddisfare le metriche individuate.

\subsection{Prodotto Portabile}
Per la portabilità del prodotto, verrà prodotta una tabella di tracciamento per identificare i requisiti prestazionali e di vincolo individuati nell'Analisi dei Requisiti. Per assicurarsi maggior portabilità dovranno essere soddisfatti queste tipologie di requisiti.
...
\subsubsection{Classe accettabile}
...
\begin{itemize}
\item \grassetto{Obiettivo Sufficiente}:[100\% soddisfacimento requisiti funzionali e obbligatori ]
...
\subsubsection{Attributi per \gloss{classe}}
\label{4.2.3}
...
\input{./Piano_di_qualifica_appendice_D.tex}
\input{./Piano_di_qualifica_appendice_E.tex}
