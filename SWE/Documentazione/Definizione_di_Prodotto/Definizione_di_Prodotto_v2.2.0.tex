...
\textbf{Verifica} & Fabio Miotto, Mattia Sorgato\\  %ED ANCHE QUI!
...
\immagine{./Diagrammi/ControllerPackage}{Diagramma del package Maap::Server::Controller}
...
Per ultima cosa, il \grassetto{@dispatcher} gestisce eventuali richieste di risorse inesistenti o errate reindirizzando il chiamante alla pagina iniziale del Client AngularJS.
 ...
 \item[Gestione Indici nel Database di analisi]
  \begin{mldescription}
    ...
    \mlitem{delete('/api/indexes/:col\_name/:index\_name', passport.checkAuthenticatedAdmin,\\datamanager.deleteIndex)} \hfill 
 \begin{description}
    \item[Parametri] \hfill
     \begin{description}
 ...
 \begin{mldescription}
  \mlitem{put('/api/signup', passport.checkAuthenticatedAdmin, usermanager.userSignup,function(req,res){res.send(200);})}
  \begin{description}
     \item[Parametri] \hfill
      \begin{description}
      ...
      \end{description}
     \item[Descrizione]
     Intercetta la richiesta di inserire un nuovo utente Admin. Verifica che essa sia autenticata come amministratore e richiama il metodo \textit{userSignup} del \grassetto{@usermanager} e alla fine invia la risposta di successo al Client.
  \end{description}
 \end{mldescription}
  
 \item[Gestione Login]
 \begin{mldescription}
 \mlitem{post('/api/forgot', passport.checkNotAuthenticated, passport.forgotPassword)}
   \begin{description}
      \item[Parametri] \hfill
       \begin{description}
...
\mlitem{post('/api/signup', passport.checkNotAuthenticated, usermanager.userSignup,passport.authenticate, function(req,res){res.send(200););})}
 \begin{description}
    \item[Parametri] \hfill
     \begin{description}
      \item['/api/signup'] \hfill \\
     ...
     \end{description}
    \item[Descrizione]
    Gestisce la richiesta di registrazione da parte di un utente non autenticato. Se l'utente associato alla richiesta non è autenticato, viene richiamato il metodo \textit{userSignup} di \grassetto{@DatabaseAnalysisManager} per creare l'utente, il metodo \textit{authenticate} di \grassetto{@passport} per eseguire l'autenticazione e poi viene inviata la risposta di successo al client.
 \end{description}
...
\mlitem{get('/api/logout', passport.checkAuthenticated, function(req, res) \{ req.session.destroy(function(err){ res.send(200);}) \})}
 \begin{description}
  \item[Parametri]
   \begin{mldescription}
    ...
    Funzione di \grassetto{@passport} che verifica che l'utente sia autenticato.
    \mlitem{function(req, res) \{ req.session.destroy(function(err){ res.send(200);}); \}} \hfill \\
...
\immagine{./Diagrammi/DatabasePackage}{Diagramma del package Maap::Server::ModelServer::Database}
\paragraph{@index}
\begin{description}
 \item[Descrizione] \hfill \\
...
\immagine{./Diagrammi/DataManagerPackage}{Diagramma del package Maap::Server::ModelServer::DataManager}
...
\item[resetQueries(req, res)] \hfill 
  ...
  \item[Parametri] \hfill
   ...
   \end{description}
  \item[Descrizione] \hfill \\
  Esegue una chiamata asincrona al metodo \textit{resetQueries} di \grassetto{@indexManager} per eliminare una lista di query. Nel caso la rimozione effettuata dall' \grassetto{@indexManager} vada a buon fine, viene ritornato al Client un codice \textit{http} di successo "200", altrimenti viene mandato il codice di errore \textit{http} 404.
  \end{description}
 ...
 \item[getTopQueries(req, res)] \hfill 
   ...
   \item[Parametri] \hfill
    ...
    \end{description}
   \item[Descrizione] \hfill \\
   Esegue una chiamata asincrona al metodo \textit{getTopQueries} di \grassetto{@indexManager} per prelevare la lista di query più utilizzate. Nel caso la richiesta effettuata al \grassetto{@indexManager} vada a buon fine, viene ritornato al Client un codice \textit{http} di successo "200", altrimenti viene mandato il codice di errore \textit{http} 404.
   \end{description}
    ...
    \item[getIndexesList(req, res)] \hfill 
      ...
      \item[Parametri] \hfill
       ...
       \end{description}
      \item[Descrizione] \hfill \\
      Esegue una chiamata asincrona al metodo \textit{getIndex} di \grassetto{@indexManager} per ottenere una lista di indici. Tale lista viene dettagliata utilizzando il metodo \textit{createIndexesList} di \grassetto{@JsonComposer} e viene ritornata al client.
      \end{description}
      ...
      \item[createIndex(req, res)] \hfill 
           ...
           \item[Parametri] \hfill
            ...
            \end{description}
           \item[Descrizione] \hfill \\
           Esegue una chiamata asincrona al metodo \textit{createIndex} di \grassetto{@indexManager} per creare l' indice. Nel caso la richiesta effettuata al \grassetto{@indexManager} vada a buon fine, viene ritornato al Client un codice \textit{http} di successo "200", altrimenti viene mandato il codice di errore \textit{http} 404.
           \end{description}
      ...
      \item[deleteIndex(req, res)] \hfill 
                 ...
                 \item[Parametri] \hfill
                  ...
                  \end{description}
                 \item[Descrizione] \hfill \\
                 Esegue una chiamata asincrona al metodo \textit{deleteIndex} di \grassetto{@indexManager} per eliminare l' indice. Nel caso la richiesta effettuata al \grassetto{@indexManager} vada a buon fine, viene ritornato al Client un codice \textit{http} di successo "200", altrimenti viene mandato il codice di errore \textit{http} 404.
                 \end{description}
...
Se essa è presente, ne ritorna il modello, altrimenti ritorna l'intero "-1".
	...
	\mlitem{getCollectionsList(find)} \hfill 
		...
		\end{description}
		\end{description}
	\mlitem{getTransormationFile((collection\_name, type, fieldName)}
	   	\begin{description}
	   		\item[Parametri] \hfill
	   					\begin{description}
	   						\item[collection\_name] \hfill \\
	...
	\mlitem{applyTransformations(collection\_name, type, documentsArray, dslArray)} \hfill 
		\begin{description}
			\item[Parametri] \hfill
				\begin{description}
				\item[collection\_name] \hfill \\
	...
	\mlitem{sortDocumentsByLabels(documents, keys)} \hfill 
		...
		\mlitem{countDocuments(model, where) } \hfill 
			\begin{description}
				\item[Parametri] \hfill
					\begin{description}
						\item[model] \hfill \\
...
Vengono contati il numero di documenti presenti nel modello passato con il parametro \textit{model} eventualmente ristretti nella selezione in base alla query passata con il parametro \textit{where}. Se il conteggio viene portato a termine viene conclusa la promessa ritornando il numero trovato come risultato. Altrimenti viene conclusa la promessa ritornando 0 come risultato.
			...
			\mlitem{findDocuments(query) } \hfill 
				\begin{description}
					\item[Parametri] \hfill
						\begin{description}
							\item[query] \hfill \\
...
\mlitem{getDocumentsForIndex(model, querySettingsy) } \hfill 
				\begin{description}
					\item[Parametri] \hfill
						\begin{description}	
						\item[model] \hfill \\
...
Viene richiesto il modello della Collection di nome \textit{collection\_name}. Successivamente vengono costruiti gli Array delle etichette e dei dati della Collection. Il nome delle varie etichette viene stabilito dal file DSL associato alla Collection, altrimenti viene preso il nome di default della colonna. A questo punto viene eseguito un controllo per stabilire se il campo {\_id} dei vari Document della Collection deve essere visibile nella visualizzazione. Viene eseguita una query nel database di analisi per ottenere tutti i Document che rispettano i parametri. Viene poi eseguita un'ulteriore query per creare localmente l'oggetto Collection con il numero di pagine specificato nel file DSL della Collection e per ottenere i Document identificati dalla pagina di visualizzazione richiesta dal Client. Se le query hanno portato ad un risultato positivo, viene richiamata la funzione \textit{callback} con argomento il JSON creato localmente. Altrimenti la \textit{callback} viene richiamata con argomento vuoto.
	...
	\mlitem{getDocumentShowEdit(collection\_name, document\_id, callback)} \hfill
		\begin{description}
			...
			\item[Dipendenza] \hfill
				\begin{itemize}
					\item JSON generato dall'interpretazione del DSL della Collection di nome 										\textit{collection\_name}.
				\end{itemize}
			\item[Descrizione] \hfill \\
...
\begin{description}
 ...
 \item[Metodi]
 	\begin{mldescription}
 		...
 		\mlitem{removeUser(id, callback)} \hfill
 		 			\begin{description}
 		 				\item[Parametri] \hfill
 		 					\begin{description}
 		 						\item[id] \hfill \\
 		 							Stringa contenente l'identificativo dell'utente che si desidera eliminare dal database.
 		 						\item[callback] \hfill \\
...
\begin{description}
 ...
 \item[Metodi]
  ...
  \mlitem{getCollectionsListFile()}
   ...
   \mlitem{getIndex(db, page, indexesPerPage, callback)}
          \begin{description}
      	 \item[Parametri] \hfill
      	  \begin{description}
      	   \item[db] \hfill \\
    ...
    \mlitem{createIndex(query\_id,  name\_index, callback)}
      ...
      \mlitem{deleteIndex(db, indexName, collectionName, callback)}
          \begin{description}
      	 \item[Parametri] \hfill
      	  \begin{description}
      	   \item[db] \hfill \\
...
\begin{description}
 ...
 \begin{mldescription}
  ...
  \mlitem{createCollection(labels, data, config)} \hfill
   ...
   \mlitem{createIndexesList(indexes,config)} \hfill
      \begin{description}
       \item[Parametro] \hfill
        \begin{description}
         \item[indexes] \hfill \\
...
\subsubsection{::DSL}
\immagine{./Diagrammi/DSLPackage}{Diagramma del package Maap::Server::ModelServer::DSL}
...
\begin{description}
 ...
 \mlitem{saveFile(buffer, fileName, filePath)} \hfill 
  	\begin{description}
     		\item[Parametro] \hfill
     			\begin{description}
     				\item[buffer] \hfill \\
...
\subsection{::View}
\immagine{./Diagrammi/ViewPackage}{Diagramma del package Maap::Client::View}
...
\begin{description}
	...
	\item[Attributi] \hfill
 	\begin{description}
		...
		Intero che rappresenta la pagina correntemente visualizzata.
		\item[\$scope.rows] \hfill \\
		Array contenente il contenuto di ogni query. Ovvero, comprende: la Collection di appartenenza, i campi su cui viene effettuata la query ed il numero di volte che essa è stata effettuata.
 	\end{description}
\end{description}
...
\begin{description}
	...
	\item[Utilizzo] \hfill \\
	In questa pagina vengono visualizzati tutti gli utenti registrati al sistema. Da qui l'utente amministratore può decidere di visualizzare un profilo utente, modificarlo o eliminarlo. Nella finestra a destra della Collection, è inoltre possibile registrare un nuovo utente.
	\item[Attributi] \hfill
	\begin{description}
		\item[\$scope.credentials] \hfill \\
...
\immagine{./Diagrammi/ControllerModelViewPackage}{Diagramma del package Maap::Client::ControllerModelView}
Package che gestisce il dialogo tra la View ed il Model del Client.

\subsubsection{::ControllerClient}
\immagine{./Diagrammi/ControllerClientPackage}{Diagramma del package Maap::Client::ControllerModelView::ControllerClient}
...
\begin{description}
 ...
 \begin{description}
  ...
  \begin{description}
  	...
  	\end{description}
  	\item[Descrizione] \hfill \\
  Funzione che elimina il Document identificato dall'indice \textit{index}. Effettua una richiesta REST di rimozione alla risorsa esportata da \grassetto{@DocumentEditService} fornendo come argomenti l'identificativo della Collection correntemente visualizzata e l'indice del Document da eliminare. \\
...
Fornisce un immediato elenco delle collection disponibili.
 ...
 \item[Dipendenze iniettate al controller] \hfill
 \begin{itemize}
  \item \$scope;
 ...
 \begin{mldescription}
  ...
  \begin{description}
  	\item[Descrizione] \hfill \\
    Funzione che richiede al server l'elenco delle collection disponibili e le inserisce nello scope, filtrando le Collection che non rispettano i parametri di ricerca di \textit{\$scope.searchbox}.
  \end{description}
 \end{mldescription}
\end{description}

\paragraph{@DocumentCtrl}
\begin{description}
 \item[Descrizione] \hfill \\
...
\begin{description}
 ...
 \begin{description}
  ...
  \begin{description}
  	...
  	\end{description}
  	\item[Descrizione] \hfill \\
  	Funzione che trasmette al server le credenziali inserite dall'utente e verifica se sono corrette. Richiama il metodo \textit{login()} di \textbf{@AuthService}, passando come parametro \textit{\$scope.credentials}.
  	In caso positivo autentica l'utente, altrimenti mostra un errore.
  \end{description}
 \end{description}
\end{description}

\paragraph{@NavBarCtrl}
\begin{description}
 \item[Descrizione] \hfill \\
...
\paragraph{@ProfileCtrl}
\begin{description}
 \item[Descrizione] \hfill \\
 Modulo che descrive il Controller della pagina di gestione del profilo.
 
 \item[Utilizzo] \hfill \\
 Controller utilizzato da AngularJS che permette all'utente di gestire il suo profilo, visualizzandone le informazioni contenute.
 ...
 \begin{description}
 	\item[Parametro] \hfill
  		\begin{description}
  			...
  			Oggetto contenente i dati sensibili di un indice.
       \end{description}
  	\item[Descrizione] \hfill \\
 ...
 \begin{description}
  ...
  \begin{description}
  	\item[Parametro] \hfill
       ...
       \end{description}
  	\item[Descrizione] \hfill \\
   Funzione che elimina il profilo identificato dall'indice \textit{\$scope.current\_document}. Effettua una richiesta REST di rimozione alla risorsa gestita da \grassetto{@UserEditService} fornendo come argomenti l'identificativo della Collection correntemente visualizzata. \\
  Se l'eliminazione ha successo viene lanciato l'aggiornamento della vista corrente con il Document eliminato, altrimenti si richiama la callback di errore.
    \end{description}
...
\immagine{./Diagrammi/DirectivesPackage}{Diagramma del package Maap::Client::ControllerModelView::Directives}
...
\end{itemize}
 ...
 Directive utilizzata per verificare che la mail inserita dall'utente durante la registrazione non sia già utilizzata.
  Se la mail è già in utilizzo non viene validato il form e non si può procedere alla registrazione.

\end{description}

\subsection{::ModelClient}
\immagine{./Diagrammi/ModelClientPackage-2}{Diagramma del package Maap::Client::ModelClient}

\subsubsection{::Services}
\immagine{./Diagrammi/ServicesPackage}{Diagramma del package Maap::Client::ModelClient::Services}
...
\begin{description}
 ...
 \item[Utilizzo] \hfill \\
  ...
  nel \grassetto{@NavBarCtrl} per effettuare il logout di un utente autenticato, tramite il metodo esposto \textit{logout}. 
 \item[Funzionalità] \hfill \\
 \emph{Dipendenze iniettate:}
 \begin{itemize}
  \item \$resource.
...
\begin{description}
 ...
 che consente di inviare una richiesta di registrazione al Server con le credenziali inserite nell'apposito form del Client. Viene inoltre esposto il metodo \grassetto{createUser} che consente all'amministratore di creare un nuovo utente dalla sua interfaccia di gestione utenti.
 \item[Funzionalità] \hfill \\
 \emph{Dipendenze iniettate:}
 \begin{itemize}
  \item \$resource.
...
\immagine{./Sequenza2}{Diagramma sequenza: Autenticazione con successo}
