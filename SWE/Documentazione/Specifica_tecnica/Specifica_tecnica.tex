...
\textbf{Distribuzione} & \parbox[t]{4cm}{\NomeGruppo{}\\Prof. Vardanega Tullio\\Prof. Cardin Riccardo}\\
\end{tabular}
\end{center}
...
\begin{itemize}
\item Semplicità: i messaggi JSON sono più corti rispetto ad altri formati di interscambio, e vengono eseguiti più velocemente dal \gloss{parser}. JSON inoltre risulta più semplice e immediato rispetto ad esempio a XML.
\end{itemize}
Svantaggi:
\begin{itemize}
...
\subsection{HTML5}
L'\gloss{HTML5} è un linguaggio di markup per la strutturazione delle pagine web.\\
...
\immagine{./Diagrammi/Installer}{Diagramma delle classi relativo alla gestione del framework da parte dell'utente sviluppatore}
...
%TEMPLATE PER DIAGRAMMA ATTIVITA'
...
\immagine{./Diagrammi/GeneralePackage}{Applicazione di Three-tier in MaaP}
\begin{itemize}
\item \grassetto{Scopo:} Il pattern Three-tier è stato scelto per suddividere...  ; 
\item \grassetto{Utilizzo:} Nel progetto MaaP ...
\end{itemize}

\subsection{Design Pattern creazionali}
\subsubsection{Singleton}
\immagine{./Diagrammi/DPSingleton}{Applicazione di Singleton in MaaP}
\begin{itemize}
...
\begin{itemize}
\item \grassetto{NodeJS:} per la realizzazione dell'applicazione web lato server;
...
\subsubsection{Three-Tier} 
\immagine{./Diagrammi/THREETIER}{Diagramma del design pattern Three-tier}
\begin{itemize}
\item \grassetto{Scopo:} Disaccoppiare le tre componenti seguenti:
\begin{itemize}
\item Client tier: strato che definisce l'interfaccia utente;
\item Business logic tier: strato che definisce la logica funzionale;
\item Database tier:strato che definisce la gestione dei dati persistenti.
\end{itemize}
\item \grassetto{Motivazione:} Il pattern Three-tier presenta diverse analogie con il pattern MVVM presentato precedentemente. Esso svolge un ruolo importante nella progettazione di applicazioni web specialmente basati su Web-service. Inoltre la suddivisione in moduli separati che seguono il paradigma client-server permette la sostituzione e/o modifica di un singolo modulo o strato indipendentemente dagli altri conferendo scalabilità e manutenibilità all'applicazione. I moduli possono essere inoltre distribuiti su diversi nodi di una rete anche eterogenea.
\item \grassetto{Applicabilità:} Il pattern Three-tier può essere utilizzo nei seguenti casi:
\begin{itemize}
\item bla bla.
\end{itemize}
\end{itemize}
...
\begin{itemize}
\item \grassetto{Scopo:} Fornire un'interfaccia unificata per un insieme di interfacce presenti in un sottosistema. Definisce un'interfaccia di livello più alto che rende il sottosistema più semplice da utilizzare;
