...
\label{2.1}
Il Responsabile di Progetto incentra su di s le responsabilità di scelta ed approvazione dei lavori. Ha inoltre il ruolo di rappresentare il gruppo nei contatti con l'esterno e durante la presentazione dei lavori.\\
...
\item La visualizzazione della repository associato al progetto;
...
\label{4.3}
Viene messa a disposizione una repository Git su GitHub per la gestione e il versionamento di codice e documenti tra i vari membri del gruppo.\\
La repository pubblica è disponibile all'indirizzo: \\
\begin{center}
...
\subsection{Branch}
\label{4.4}
Nella repository saranno disponibili vari branch per favorire lo sviluppo del codice da parte degli sviluppatori.
...
\item I ruoli di progetto (es. Analista) dovranno essere scritti in corsivo per differenziarli dal riferimento generale alla figura professionale;
...
\item Gli elenchi puntati sono formati da dei pallini neri pieni, tranne quando si deve elencare una sequenza numerata di istruzioni da fare in un ordine stabilito.
\end{itemize}
...
\begin{itemize}
\item Enqueued: il ticket è stato creato ed è in attesa di essere assegnato ad un X per la risoluzione del problema;
...
Le definizioni dei termini devono essere quanto più semplici e chiare possibile, e devono cercare di evitare ricorsioni o termini da glossario.
